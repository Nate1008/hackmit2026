...
def get_image_data_dav3_workaround(
    ...
    """Load multiple already-rectified DA3 video views.

    The original experimental path repeated view zero. Raw-video inference
    supplies a mask, intrinsics and pose per visible frame, so retain the most
    informative views and preserve their temporal ordering.
    """
    view_count = len(pkl_sample["image_data"])
    required = ("mask_data", "camera_params", "camera_to_worlds")
    if view_count == 0 or any(len(pkl_sample[key]) != view_count for key in required):
        raise ValueError("DA3 sample has inconsistent per-view data")

    decoded_masks = []
    for encoded_mask in pkl_sample["mask_data"]:
        with Image.open(io.BytesIO(encoded_mask)) as image:
            decoded_masks.append(np.array(image.convert("L")))
    ranked = sorted(
        range(view_count),
        key=lambda index: np.count_nonzero(decoded_masks[index]),
        reverse=True,
    )[: min(num_views, view_count)]
    selected = sorted(ranked)
    if len(selected) < num_views:
        selected.extend(
            selected[i % len(selected)] for i in range(num_views - len(selected))
        )

    rectified_images = []
    rectified_masks = []
    for index in selected:
        with Image.open(io.BytesIO(pkl_sample["image_data"][index])) as image:
            rectified_images.append(np.array(image.convert("RGB" if is_rgb else "L")))
        rectified_masks.append(decoded_masks[index])
    rectified_camera_params = convert_to_4x4(
        [np.asarray(pkl_sample["camera_params"][index]) for index in selected]
    )
    for im_idx in range(len(rectified_masks)):
        rectified_masks[im_idx] = np.rot90(rectified_masks[im_idx], 1)
    rectified_masks = np.stack(rectified_masks)
    ...
    camera_to_worlds = np.stack(
        [np.asarray(pkl_sample["camera_to_worlds"][index]) for index in selected],
        axis=0,
    )
